...
const statusConfig = {
  ...
  Shipped: { icon: Truck, color: "text-teal-400", bg: "bg-teal-400/10" },
          ...
          <div className="text-center py-20">
            ...
            <p className="text-muted-foreground">
              {filter === "All"
                ...
                : t("orders.noFilterOrders", { filter: filter.toLowerCase() })}
            </p>
          </div>
        ) : (
                          ...
                          >
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                {t("orders.qty", { quantity: item.quantity, price: Number(item.price).toFixed(2) })}
                              </p>
                              <span
                                className={`inline-flex items-center gap-1 mt-1 text-xs ${itemCfg.color}`}
                              >
                                <ItemStatusIcon className="w-3 h-3" />
                                {statusLabels[item.item_status] || statusLabels.Processing}
                              </span>
                            </div>
                          </div>
                        );
